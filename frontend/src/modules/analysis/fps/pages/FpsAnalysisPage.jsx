...
export default function FpsAnalysisPage() {
  ...
  const handleSubmitRecalculate = (e) => {
    ...
    if (reportWindow) {
      ...
      reportWindow.document.body.innerHTML = "<p style=\"font-family: Arial, sans-serif; padding: 24px;\">Generando recalculo de precios unitarios FPS...</p>";
    }
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ["fps-items", { page, perPage, search }],
    queryFn: () => itemsService.fpsList({ page, perPage, search }),
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    if (reportWindow) {
      reportWindow.document.title = "Generando PDF";
      reportWindow.document.body.innerHTML = "<p style=\"font-family: Arial, sans-serif; padding: 24px;\">Generando analisis de precios unitarios FPS...</p>";
    }
        ...
        <CardHeader className="gap-4 border-b border-border/70 bg-muted/25">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl tracking-[-0.04em]">Análisis FPS</CardTitle>
              </div>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="flex flex-col gap-6 p-5 sm:p-6">
          ...
          {isLoading && (
            <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/70 p-8 text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Cargando items FPS...
            </div>
          )}
          ...
          {isError && (
            <Alert variant="destructive" className="rounded-2xl">
              <AlertDescription>
                {error?.response?.data?.message || "No se pudieron cargar los items FPS."}
              </AlertDescription>
            </Alert>
          )}
                    ...
                    {items.length === 0 && (
                      <tr>
                        <td colSpan={8} className="px-5 py-8 text-center text-muted-foreground">
                          No se encontraron items FPS para los filtros seleccionados.
                        </td>
                      </tr>
                    )}
                  </tbody>
