...
export default function FndrAnalysisPage() {
  ...
  const handleSubmitRecalculate = (e) => {
    ...
    openPdfViewer(itemsService.priceRecalculationPdfUrl({
      ...
      title: "Recálculo precio item FNDR",
      errorMessage: "No se pudo generar el recálculo de precio item FNDR.",
    });
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    try {
      openPdfViewer(itemsService.legacyUnitPriceAnalysisPdfUrl(item.id_item, "fndr"), {
        title: "Análisis de precio unitario FNDR",
        errorMessage: "No se pudo generar el análisis de precio unitario FNDR.",
      });
    } catch (mutationError) {
      setReportFeedback({
        type: "error",
        message: mutationError?.response?.data?.message || mutationError?.message || "No se pudo abrir el analisis de precios unitarios FNDR.",
      });
    } finally {
  ...
  const handleDownloadUnitPriceAnalysisXlsx = (item) => {
    if (!item?.id_item) return;
    setReportFeedback(null);
    downloadUrl(itemsService.legacyUnitPriceAnalysisXlsxUrl(item.id_item, "fndr"));
  };
