...
export default function UpreAnalysisPage() {
  ...
  const handleSubmitRecalculate = (e) => {
    ...
    if (reportWindow) {
      ...
      reportWindow.document.body.textContent = "Generando recalculo de precios unitarios UPRE...";
    }
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    if (reportWindow) {
      reportWindow.document.title = "Generando PDF";
      reportWindow.document.body.textContent = "Generando analisis de precios unitarios UPRE...";
    }

    try {
      const pdfResponse = await itemsService.downloadLegacyUnitPriceAnalysisPdf(item.id_item, "upre");
      const pdfBlob = pdfResponse instanceof Blob
        ? pdfResponse
        : new Blob([pdfResponse], { type: "application/pdf" });
