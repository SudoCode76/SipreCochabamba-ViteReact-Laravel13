...
export default function UpreAnalysisPage() {
  ...
  const handleSubmitRecalculate = (e) => {
    ...
    openPdfViewer(itemsService.priceRecalculationPdfUrl({
      ...
      title: "Recálculo precio item UPRE",
      errorMessage: "No se pudo generar el recálculo de precio item UPRE.",
    });
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    ...
    queryFn: () => itemsService.upreList({ page, perPage, search }),
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    try {
      openPdfViewer(itemsService.legacyUnitPriceAnalysisPdfUrl(item.id_item, "upre"), {
        title: "Análisis de precio unitario UPRE",
        errorMessage: "No se pudo generar el análisis de precio unitario UPRE.",
      });
    } catch (mutationError) {
      setReportFeedback({
        type: "error",
        message: mutationError?.response?.data?.message || mutationError?.message || "No se pudo abrir el analisis de precios unitarios UPRE.",
      });
    } finally {
  ...
  const handleDownloadUnitPriceAnalysisXlsx = (item) => {
    if (!item?.id_item) return;
    setReportFeedback(null);
    downloadUrl(itemsService.legacyUnitPriceAnalysisXlsxUrl(item.id_item, "upre"));
  };
        ...
        <CardHeader className="gap-4 border-b border-border/70 bg-muted/25">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl tracking-[-0.04em]">Análisis UPRE</CardTitle>
              </div>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="flex flex-col gap-6 p-5 sm:p-6">
          ...
          {isLoading && (
            <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/70 p-8 text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Cargando items UPRE...
            </div>
          )}
          ...
          {isError && (
            <Alert variant="destructive" className="rounded-2xl">
              <AlertDescription>
                {error?.response?.data?.message || "No se pudieron cargar los items UPRE."}
              </AlertDescription>
            </Alert>
          )}
                    ...
                    {items.length === 0 && (
                      <tr>
                        <td colSpan={8} className="px-5 py-8 text-center text-muted-foreground">
                          No se encontraron items UPRE para los filtros seleccionados.
                        </td>
                      </tr>
                    )}
                  </tbody>
