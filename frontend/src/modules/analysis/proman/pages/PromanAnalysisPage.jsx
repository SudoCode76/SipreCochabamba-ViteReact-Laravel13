...
export default function PromanAnalysisPage() {
  ...
  const handleSubmitRecalculate = (e) => {
    ...
    if (reportWindow) {
      ...
      reportWindow.document.body.innerHTML = "<p style=\"font-family: Arial, sans-serif; padding: 24px;\">Generando recalculo de precios unitarios PROMAN...</p>";
    }
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ["proman-items", { page, perPage, search }],
    queryFn: () => itemsService.promanList({ page, perPage, search }),
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    if (reportWindow) {
      reportWindow.document.title = "Generando PDF";
      reportWindow.document.body.innerHTML = "<p style=\"font-family: Arial, sans-serif; padding: 24px;\">Generando analisis de precios unitarios Proman...</p>";
    }

    try {
      const pdfResponse = await itemsService.downloadLegacyUnitPriceAnalysisPdf(item.id_item, "proman");
      const pdfBlob = pdfResponse instanceof Blob
        ? pdfResponse
        : new Blob([pdfResponse], { type: "application/pdf" });
      ...
      setReportFeedback({
        type: "error",
        message: mutationError?.response?.data?.message || mutationError?.message || "No se pudo abrir el analisis de precios unitarios Proman.",
      });
    } finally {
        ...
        <CardHeader className="gap-4 border-b border-border/70 bg-muted/25">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              <div className="flex size-11 items-center justify-center rounded-2xl bg-rose-600 text-white">
                <TrendingUp className="size-5" />
              </div>
              <div>
                <CardTitle className="text-2xl tracking-[-0.04em]">Análisis Proman</CardTitle>
              </div>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="flex flex-col gap-6 p-5 sm:p-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <form className="flex w-full max-w-sm flex-col gap-2" onSubmit={handleSearchSubmit}>
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Buscar item Proman"
          ...
          {isLoading && (
            <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/70 p-8 text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Cargando items Proman...
            </div>
          )}
          ...
          {isError && (
            <Alert variant="destructive" className="rounded-2xl">
              <AlertDescription>
                {error?.response?.data?.message || "No se pudieron cargar los items Proman."}
              </AlertDescription>
            </Alert>
          )}
                    ...
                    {items.length === 0 && (
                      <tr>
                        <td colSpan={8} className="px-5 py-8 text-center text-muted-foreground">
                          No se encontraron items Proman para los filtros seleccionados.
                        </td>
                      </tr>
                    )}
                  </tbody>
