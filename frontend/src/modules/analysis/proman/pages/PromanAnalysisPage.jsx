...
export default function PromanAnalysisPage() {
  ...
  const handleOpenUnitPriceAnalysisPdf = async (item) => {
    ...
    try {
      openPdfViewer(itemsService.legacyUnitPriceAnalysisPdfUrl(item.id_item, "proman"), {
        title: "Análisis de precio unitario PROMAN",
        errorMessage: "No se pudo generar el análisis de precio unitario PROMAN.",
      });
    } catch (mutationError) {
      setReportFeedback({
        type: "error",
