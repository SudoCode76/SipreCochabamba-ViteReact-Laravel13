...
export default function FpsCalculationPercentagesPage() {
  ...
  const contextQuery = useQuery({
    queryKey: ["fps-calculation-context"],
    queryFn: fpsCalculationPercentagesService.context,
  });

  const listQuery = useQuery({
    queryKey: ["fps-calculation-percentages", { page, perPage, searchTerm, statusFilter }],
    queryFn: () => fpsCalculationPercentagesService.list({
      page,
      perPage,
  ...
  const openEdit = async (id) => {
    ...
    try {
      const detail = await queryClient.fetchQuery({
        queryKey: ["fps-calculation-detail", id],
        queryFn: () => fpsCalculationPercentagesService.getById(id),
      });
      ...
      setFeedback({
        type: "error",
        message: error.response?.data?.message || "No se pudo cargar el detalle del porcentaje FPS.",
      });
      closeForm();
    } finally {
    ...
    onError: (error) => {
      ...
      setFeedback({
        ...
        message: error.response?.status === 403
          ? "Acceso denegado para crear porcentajes FPS."
          : error.response?.data?.message || "No se pudo crear el porcentaje FPS.",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setFeedback({
        type: "error",
        message: error.response?.status === 403
          ? "Acceso denegado para editar porcentajes FPS."
          : error.response?.data?.message || "No se pudo actualizar el porcentaje FPS.",
      });
    },
  });
        ...
        <CardHeader className="gap-4 border-b border-border/70 bg-muted/25">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl tracking-[-0.04em]">Porcentaje de Cálculo FPS</CardTitle>
                <CardDescription>
                  Administración de porcentajes de cálculo FPS para los procesos del sistema.
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="flex flex-col gap-6 p-5 sm:p-6">
          ...
          {listQuery.isLoading && (
            <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/70 p-8 text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Cargando porcentajes FPS...
            </div>
          )}
            ...
            <Alert variant="destructive" className="rounded-2xl">
              <AlertDescription>
                {listQuery.error?.response?.status === 403
                  ? "No tienes permisos para acceder a Porcentaje de Cálculo FPS."
                  : listQuery.error?.response?.data?.message || "No se pudieron cargar los porcentajes FPS."}
              </AlertDescription>
            </Alert>
          )}
                      ...
                      {items.length === 0 && (
                        <tr>
                          <td colSpan={6} className="px-5 py-8 text-center text-muted-foreground">
                            No se encontraron porcentajes FPS para los filtros seleccionados.
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <CardTitle className="text-2xl tracking-[-0.04em]">
                      {isEditing ? "Editar FPS" : "Registrar FPS"}
                    </CardTitle>
                    <CardDescription>
                      {isEditing
                        ? "Actualiza el porcentaje de cálculo FPS seleccionado."
                        : "Registra un nuevo porcentaje de cálculo FPS."}
                    </CardDescription>
                  </div>
                  ...
                  <form className="flex flex-col gap-5" onSubmit={handleSubmitForm}>
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      <Button type="submit" className="rounded-full bg-foreground text-background hover:bg-foreground/90" disabled={isSubmitting}>
                        ...
                        {isEditing ? "Guardar cambios" : "Registrar FPS"}
                      </Button>
                    </div>
                  </form>
                )}
