...
export default function ObrasCalculationPercentagesPage() {
  ...
  const contextQuery = useQuery({
    queryKey: ["obras-calculation-context"],
    queryFn: obrasCalculationPercentagesService.context,
  });

  const listQuery = useQuery({
    queryKey: ["obras-calculation-percentages", { page, perPage, searchTerm, statusFilter }],
    queryFn: () => obrasCalculationPercentagesService.list({
      page,
      perPage,
  ...
  const openEdit = async (id) => {
    ...
    try {
      const detail = await queryClient.fetchQuery({
        queryKey: ["obras-calculation-detail", id],
        queryFn: () => obrasCalculationPercentagesService.getById(id),
      });
      ...
      setFeedback({
        type: "error",
        message: error.response?.data?.message || "No se pudo cargar el detalle del porcentaje OBRAS PUBLICAS.",
      });
      closeForm();
    } finally {
    ...
    onError: (error) => {
      ...
      setFeedback({
        ...
        message: error.response?.status === 403
          ? "Acceso denegado para crear porcentajes OBRAS PUBLICAS."
          : error.response?.data?.message || "No se pudo crear el porcentaje OBRAS PUBLICAS.",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setFeedback({
        type: "error",
        message: error.response?.status === 403
          ? "Acceso denegado para editar porcentajes OBRAS PUBLICAS."
          : error.response?.data?.message || "No se pudo actualizar el porcentaje OBRAS PUBLICAS.",
      });
    },
  });
        ...
        <CardHeader className="gap-4 border-b border-border/70 bg-muted/25">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl tracking-[-0.04em]">% OBRAS PUBLICAS</CardTitle>
                <CardDescription>
                  Administración de porcentajes de cálculo de Obras Públicas para los procesos del sistema.
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="flex flex-col gap-6 p-5 sm:p-6">
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_220px_140px] xl:items-end">
            ...
            {listQuery.isFetching && !listQuery.isLoading && (
              <div className="xl:col-span-3 flex items-center gap-2 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm text-muted-foreground">
                ...
                Buscando porcentajes OBRAS PUBLICAS...
              </div>
            )}
          </div>
          ...
          {listQuery.isLoading && (
            <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/70 p-8 text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Cargando porcentajes OBRAS PUBLICAS...
            </div>
          )}
            ...
            <Alert variant="destructive" className="rounded-2xl">
              <AlertDescription>
                {listQuery.error?.response?.status === 403
                  ? "No tienes permisos para acceder a % OBRAS PUBLICAS."
                  : listQuery.error?.response?.data?.message || "No se pudieron cargar los porcentajes OBRAS PUBLICAS."}
              </AlertDescription>
            </Alert>
          )}
                      ...
                      {items.length === 0 && (
                        <tr>
                          <td colSpan={6} className="px-5 py-8 text-center text-muted-foreground">
                            No se encontraron porcentajes OBRAS PUBLICAS para los filtros seleccionados.
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <CardTitle className="text-2xl tracking-[-0.04em]">
                      {isEditing ? "Editar % OBRAS PUBLICAS" : "Registrar % OBRAS PUBLICAS"}
                    </CardTitle>
                    <CardDescription>
                      {isEditing
                        ? "Actualiza el porcentaje de cálculo de Obras Públicas seleccionado."
                        : "Registra un nuevo porcentaje de cálculo de Obras Públicas."}
                    </CardDescription>
                  </div>
                  ...
                  <form className="flex flex-col gap-5" onSubmit={handleSubmitForm}>
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      <Button type="submit" className="rounded-full bg-foreground text-background hover:bg-foreground/90" disabled={isSubmitting}>
                        ...
                        {isEditing ? "Guardar cambios" : "Registrar % OBRAS PUBLICAS"}
                      </Button>
                    </div>
                  </form>
                )}
