...
class FpsItemApiTest extends TestCase
{
    ...
    public function test_user_with_fps_view_permission_can_get_context_with_functional_permissions(): void
    {
        $this->createGroup();
        $this->createSubgroup();
        $this->createUnitMeasure();

        $user = $this->createFpsUserWithPermissions(['FPS']);

        Sanctum::actingAs($user);

        $response = $this->getJson('/api/v1/items/fps/context');
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.meta.screen', 'items/fps')
            ->assertJsonPath('data.meta.mode', 'fps');
    }

    public function test_fps_list_returns_paginated_filtered_items(): void
    {
        ...
        $response = $this->getJson('/api/v1/items/fps?search=ITEM&group_id=1&subgroup_id=1&status=AC&per_page=10');

        $response->assertOk()
            ->assertJsonPath('success', true)
            ->assertJsonPath('data.meta.total', 1)
            ->assertJsonPath('data.items.0.id_item', 1)
            ->assertJsonPath('data.items.0.calculated_price', 61.9885);
    }

    public function test_fps_price_analysis_returns_current_breakdown(): void
    {
        Sanctum::actingAs($this->createLegacyAuthUser());
        ...
        $response = $this->getJson('/api/v1/items/1/price-analysis?mode=fps');
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.totals.total_price', 61.9885);
    }

    public function test_fps_price_recalculation_uses_latest_logs_until_date(): void
    {
        Sanctum::actingAs($this->createLegacyAuthUser());
        ...
        $response = $this->postJson('/api/v1/items/1/price-recalculation?mode=fps', [
            'fecha' => '2026-04-30',
        ]);
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.totals.total_price', 49.3299);
    }

    private function createFpsUserWithPermissions(array $functionNames): User
    {
