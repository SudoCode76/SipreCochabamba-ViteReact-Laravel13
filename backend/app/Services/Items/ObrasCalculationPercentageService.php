...
class ObrasCalculationPercentageService
{
    ...
    public function context(User $user): array
    {
        return [
            ...
            'permissions' => $this->permissions->resolve($user, 'OBRAS_PUBLICAS'),
            'filters' => ['search', 'description', 'code', 'status', 'page', 'per_page'],
            'endpoints' => [
                'list' => '/api/v1/calculation-percentages/obras',
                'show' => '/api/v1/calculation-percentages/obras/{id}',
                'create' => '/api/v1/calculation-percentages/obras',
                'update' => '/api/v1/calculation-percentages/obras/{id}',
            ],
        ];
    }
    ...
    {
        $query = ObrasCalculationPercentage::query();
    ...
    public function create(StoreCalculationPercentageRequest $request, User $user): ObrasCalculationPercentage
    {
        ...
        $percentage = ObrasCalculationPercentage::query()->create([
            'codigo' => $code,
        ...
        $this->registerAudit($user, $request->ip(), 'Registro de porcentaje de calculo OBRAS '.$percentage->descripcion);

        return $percentage;
    }

    public function update(UpdateCalculationPercentageRequest $request, ObrasCalculationPercentage $percentage, User $user): ObrasCalculationPercentage
    {
        $code = trim($request->string('codigo')->toString());
        $description = trim($request->string('descripcion')->toString());
        ...
        $this->registerAudit($user, $request->ip(), 'Actualizacion de porcentaje de calculo OBRAS '.$percentage->descripcion);
    ...
    private function ensureUniqueCode(string $code, ?int $ignoreId = null): void
    {
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'codigo' => ['Ya existe un porcentaje de calculo Obras con el mismo codigo.'],
            ]);
        }
    }
    ...
    private function ensureUniqueDescription(string $description, ?int $ignoreId = null): void
    {
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'descripcion' => ['Ya existe un porcentaje de calculo Obras con la misma descripcion.'],
            ]);
        }
    }
